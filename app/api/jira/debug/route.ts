...
export async function POST(request: NextRequest) {
  try {
    ...
    for (const test of searchTests) {
      try {
        const response = await fetch(`${credentials.base_url}/rest/api/3/search/jql?jql=${encodeURIComponent(test.jql)}&maxResults=5&fields=key,summary,description,status,priority,assignee,reporter,project,issuetype,created,updated`, {
          headers: {
            'Authorization': `Basic ${Buffer.from(`${credentials.user_email_integration}:${credentials.api_token}`).toString('base64')}`,
            'Accept': 'application/json'
          }
