...
import { Search, Eye, Filter, Calendar, User as UserIcon, RefreshCw, Download } from 'lucide-react';
  ...
  const loadTeamWorkReports = async () => {
    try {
      ...
      if (teamMemberEmails.length === 0) {
        const workLogFilters: any = {
          user_email: currentUser.email
        };

        // Apply additional filters
        if (filters.user) {
          workLogFilters.user_email = filters.user;
        }
        if (filters.project) {
          workLogFilters.project_id = filters.project;
        }
        if (filters.dateFrom) {
          workLogFilters.start_time_gte = filters.dateFrom;
        }
        if (filters.dateTo) {
          workLogFilters.start_time_lte = filters.dateTo;
        }

        const response = await supabaseClient.getTeamWorkLogs(workLogFilters, {
        ...
        if (response.error) {
          console.error('Error loading work reports:', response.error);
        ...
        return;
             ...
             // User is in teams, show work logs
      const workLogFilters: any = {
        user_email: teamMemberEmails
      };
      ...
      {totalPages > 1 && (
        <div className="flex items-center justify-between mt-6">
          <div className="text-sm text-neutral-700 dark:text-neutral-300">
            Showing {((currentPage - 1) * itemsPerPage) + 1} to {Math.min(currentPage * itemsPerPage, workLogs.length)} of {workLogs.length} results
          </div>
