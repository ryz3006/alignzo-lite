...
import CreateTaskModalOptimized from '@/components/kanban/CreateTaskModalOptimized';
...
export default function KanbanBoardPageOptimized() {
  ...
  const initializePage = async () => {
    try {
      ...
      await Promise.all([
        ...
        loadUserTeams(currentUser.email!)
      ]);
  ...
  const loadUserProjects = async (userEmail: string) => {
    try {
      const projectsResponse = await getUserAccessibleProjectsOptimized(userEmail);
  ...
  // Load kanban board when project or team changes
  useEffect(() => {
  ...
  const loadKanbanBoard = useCallback(async (forceRefresh = false) => {
    ...
    try {
      ...
      if (response.success) {
        ...
        // Cache the data
        setCachedData(`kanban-${selectedProject.id}-${selectedTeam}`, response.data, 30000);
  ...
  }, [selectedProject, selectedTeam, lastFetchTime, boardLoaded]);
  ...
  const handleDragEnd = async (result: DropResult) => {
    ...
    try {
      ...
      const response = await moveTaskOptimized(taskId, newColumnId, newSortOrder, user.email!);
      if (!response.success) {
  ...
  const handleCreateTask = async (taskData: CreateTaskForm) => {
    ...
    try {
      const response = await createKanbanTaskOptimized({
        ...taskData,
      ...
      if (response.success) {
        ...
        loadKanbanBoard(true); // Refresh to show new task
  ...
  const filteredTasks = useCallback((tasks: KanbanTaskWithDetails[]) => {
          ...
          <div className="bg-neutral-50 dark:bg-neutral-700 rounded-lg p-4">
            <div className="flex items-center space-x-4">
              ...
              <button
                ...
                disabled={!selectedProject || !selectedTeam || isRefreshing || isLoadingBoard}
              ...
              <button
                onClick={handleRefresh}
                disabled={isRefreshing || isLoadingBoard}
