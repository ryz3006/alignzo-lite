-- JIRA Project Mapping Table
-- This table stores mappings between internal dashboard projects and JIRA projects
-- One dashboard project can map to multiple JIRA projects

-- Drop existing objects if they exist (for idempotency)
DROP TRIGGER IF EXISTS update_jira_project_mappings_updated_at ON jira_project_mappings;
...
-- Create RLS policies
-- Policy for SELECT: Users can see mappings for their integrations
...
-- Policy for INSERT: Users can create mappings for their integrations
...
-- Policy for UPDATE: Users can update their own mappings
...
-- Policy for DELETE: Users can delete their own mappings
...
COMMENT ON COLUMN jira_project_mappings.dashboard_project_id IS 'Reference to internal dashboard project';
COMMENT ON COLUMN jira_project_mappings.jira_project_key IS 'JIRA project key (e.g., PROJ, DEV, etc.)';
COMMENT ON COLUMN jira_project_mappings.jira_project_name IS 'JIRA project display name';
COMMENT ON COLUMN jira_project_mappings.integration_user_email IS 'Email of user who created the JIRA integration';
